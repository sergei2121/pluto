  ...
  return (
    <span className="relative inline-flex h-2.5 w-2.5 shrink-0">
      {pulse && status !== 'unknown' && (
        <span className={cls('absolute inset-0 rounded-full opacity-40', m.dot, status === 'down' ? 'dot-crit' : 'dot-live')} />
      )}
  ...
  return (
    <button
      ...
      className={cls(
        ...
        checked ? 'border-vio/60 bg-vio/30' : 'border-line bg-raised',
...
export function Field({ label, children, hint }: { label: string; children: ReactNode; hint?: string }) {
  return (
    <label className="block">
      ...
      {children}
      {hint && <span className="mt-1 block text-[11px] leading-snug text-dim/80">{hint}</span>}
    </label>
  );
}
  ...
  return (
    <div className="relative">
      <svg viewBox={`0 0 ${w} ${height}`} className="w-full" preserveAspectRatio="none" style={{ height }}>
        ...
        {[0.25, 0.5, 0.75].map((f) => (
          <line key={f} x1="0" x2={w} y1={height * f} y2={height * f} stroke="#242b4a" strokeWidth="0.6" strokeDasharray="3 5" />
        ))}
