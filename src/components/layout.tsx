...
import type { Route, Severity } from '../lib/types';
...
const TOAST_META: Record<Severity, { icon: ReactNode; cls: string }> = {
      ...
      {list.map((t) => {
        ...
        return (
          <div key={t.id} className={cls('toast-in pointer-events-auto flex items-start gap-2.5 rounded-lg border bg-panel/95 px-3.5 py-3 shadow-[0_12px_40px_-8px_rgba(0,0,0,.8)] backdrop-blur', m.cls)}>
            ...
            <button onClick={() => { useToasts.list = useToasts.list.filter((x) => x.id !== t.id); }} className="text-dim transition-colors hover:text-ink">
...
function filterNav(user: ReturnType<typeof useCurrentUser>) {
  return NAV.filter((n) => {
...
export function Sidebar() {
  ...
  return (
    <aside className="relative z-10 hidden h-screen w-[228px] shrink-0 flex-col border-r border-line bg-deep/80 backdrop-blur-sm lg:flex">
      <div className="flex items-center gap-3 px-5 pb-5 pt-6">
        <Orbit className="h-9 w-9 text-vio drop-shadow-[0_0_12px_rgba(143,125,240,.45)]" />
        <div>
          ...
          return (
            <button key={n.route} onClick={() => store.nav(n.route)}
              ...
              {n.route === 'dashboard' && critCount > 0 && (
                <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-crit/15 px-1.5 font-mono text-[10.5px] font-bold text-crit ring-1 ring-crit/30">{critCount}</span>
              )}
            </button>
          );
        })}
...
function Clock() {
  ...
  return <span className="hidden font-mono text-[12px] tabular-nums text-dim md:inline">{fmtClock(now)}</span>;
}
...
export function Topbar() {
  ...
  const results = useMemo(() => {
    ...
    const ags = agents.filter((a) => a.name.toLowerCase().includes(query) || a.ip.includes(query) || a.tags.some((t) => tagIds.includes(t))).slice(0, 4);
    return { devs, ags };
  ...
  return (
    <header className="relative z-10 flex h-[60px] shrink-0 items-center gap-4 border-b border-line bg-deep/60 px-4 backdrop-blur-sm md:px-6">
      <Orbit className="h-6 w-6 text-vio lg:hidden" />
      <h1 className="truncate font-display text-[15px] font-semibold tracking-wide text-ink">{TITLES[route]}</h1>

      <div ref={boxRef} className="relative ml-auto hidden w-[320px] md:block">
        <div className={cls('flex items-center gap-2 rounded-lg border bg-raised/70 px-3 py-2 transition-all duration-200', focus ? 'border-vio/60 shadow-[0_0_0_3px_rgba(143,125,240,.12)]' : 'border-line')}>
          ...
          <input value={q} onChange={(e) => { setQ(e.target.value); setFocus(true); }} onFocus={() => setFocus(true)}
            placeholder="Поиск: IP, имя или тег…" className="w-full bg-transparent text-[13px] text-ink outline-none placeholder:text-dim/80" />
      ...
      <div className="hidden items-center gap-2 rounded-lg border border-line bg-raised/50 px-3 py-1.5 sm:flex">
...
function MobileNav() {
  const route = usePluto((s) => s.route);
  const user = useCurrentUser();
  const items = filterNav(user).filter((n) => n.route !== 'deploy');
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-line bg-deep/95 backdrop-blur lg:hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      {items.map((n) => (
        <button key={n.route} onClick={() => store.nav(n.route)}
          className={cls('flex flex-1 flex-col items-center gap-0.5 py-2 text-[9px] font-semibold transition-colors', route === n.route ? 'text-vio' : 'text-dim')}>
          {n.icon}
          <span className="truncate px-1">{n.label}</span>
        </button>
      ))}
      <button onClick={() => store.logout()} className="flex flex-1 flex-col items-center gap-0.5 py-2 text-[9px] font-semibold text-dim">
        <LogOut className="h-[17px] w-[17px]" /><span>Выход</span>
      </button>
    </nav>
  );
}
  ...
  return (
    <div className="relative flex h-screen overflow-hidden bg-void text-ink">
      ...
      <div className="relative z-10 flex min-w-0 flex-1 flex-col">
        <Topbar />
        <main className="scroll-thin min-h-0 flex-1 overflow-y-auto pb-20 lg:pb-0">
