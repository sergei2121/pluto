...
import {
  Globe, AlertTriangle, Activity, Monitor, Star, Server, Rocket, LayoutGrid, Check, Bell, Crosshair,
} from 'lucide-react';
import { Panel, StatusDot, STATUS_META, Sparkbar, Seg, TypeBadge, EmptyState } from '../components/ui';
import { store, useCurrentUser, usePluto, visibleAgents, visibleDevices, FAVORITES_LIMIT } from '../lib/store';
  ...
  return (
    <li className="ev-in relative flex gap-3 border-b border-line/40 py-2.5 pl-3 pr-1 last:border-0">
      ...
      <div className="min-w-0 flex-1">
        ...
        <span className="mt-0.5 block font-mono text-[10px] text-dim/80">{new Date(e.ts).toLocaleTimeString('ru-RU')}</span>
      </div>
    </li>
  );
});
  ...
  return (
    <div className="group cursor-pointer rounded-lg border border-line bg-raised/50 p-3.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-vio/40 hover:bg-raised/80"
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        <TypeBadge t={d.type} />
        <span className={cls('font-mono text-[15px] font-bold tabular-nums', m.text)}>{d.status === 'down' ? 'СБОЙ' : fmtMs(d.latency)}</span>
      </div>
  ...
  return (
    <div className="group cursor-pointer rounded-lg border border-line bg-raised/50 p-3.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-vio/40 hover:bg-raised/80"
      ...
      <div className="mt-2.5 grid grid-cols-3 gap-2 text-center">
        <div><div className={cls('font-mono text-[15px] font-bold', off ? 'text-dim' : 'text-vio')}>{off || g?.cpu == null ? '—' : `${Math.round(g.cpu)}%`}</div><div className="text-[8.5px] font-bold uppercase tracking-wider text-dim">CPU</div></div>
        <div><div className={cls('font-mono text-[15px] font-bold', off ? 'text-dim' : 'text-warn')}>{off || g?.cput == null ? '—' : `${Math.round(g.cput)}°`}</div><div className="text-[8.5px] font-bold uppercase tracking-wider text-dim">t°C CPU</div></div>
        <div><div className={cls('font-mono text-[15px] font-bold', off ? 'text-dim' : 'text-blu')}>{a.latency != null ? a.latency : '—'}</div><div className="text-[8.5px] font-bold uppercase tracking-wider text-dim">пинг, мс</div></div>
        <div><div className={cls('font-mono text-[15px] font-bold', off ? 'text-dim' : 'text-mint')}>{off || g?.ram == null ? '—' : `${Math.round(g.ram)}%`}</div><div className="text-[8.5px] font-bold uppercase tracking-wider text-dim">RAM</div></div>
        <div><div className={cls('font-mono text-[13px] font-bold leading-[19px]', off ? 'text-dim' : 'text-ok')}>{off || !g ? '—' : `↓${fmtNet(g.rx)}`}</div><div className="text-[8.5px] font-bold uppercase tracking-wider text-dim">сеть RX</div></div>
        <div><div className={cls('font-mono text-[15px] font-bold', off ? 'text-dim' : 'text-[#d98bb0]')}>{off || g?.ssdt == null ? '—' : `${Math.round(g.ssdt)}°`}</div><div className="text-[8.5px] font-bold uppercase tracking-wider text-dim">t° SSD</div></div>
      </div>
...
const FavAgentPingsCard = memo(function FavAgentPingsCard({ id }: { id: string }) {
  ...
  return (
    <div className="group cursor-pointer rounded-lg border border-line bg-raised/50 p-3.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-mint/40 hover:bg-raised/80"
      onClick={() => store.nav('agent-pings')}>
      <div className="flex items-center gap-2">
        <Crosshair className="h-4 w-4 shrink-0 text-mint" />
        <span className="min-w-0 flex-1 truncate text-[13px] font-semibold text-ink">{a.name}</span>
        <button onClick={(e) => { e.stopPropagation(); store.toggleAgentPingsFav(a.id); }} className="text-warn transition-transform hover:scale-110" title="Убрать из избранного">
          <Star className="h-4 w-4 fill-warn" strokeWidth={1.4} />
        </button>
      </div>
      <div className="mt-2.5 grid grid-cols-3 gap-2 text-center">
        <div><div className="font-mono text-[15px] font-bold text-ink">{st.total}</div><div className="text-[8.5px] font-bold uppercase tracking-wider text-dim">всего</div></div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <KpiTile label="Устройства в сети" value={devices.length} accent="#8f7df0" delay={0} icon={<Globe className="h-[18px] w-[18px]" />}
          sub={devices.length ? `${up} доступно · ${devices.length - up} прочее` : 'устройства ещё не добавлены'} onClick={() => store.nav('devices')} />
        <KpiTile label="В аварии" value={down} accent="#e07a80" delay={60} icon={<AlertTriangle className="h-[18px] w-[18px]" />}
          sub={down ? 'потеря связи · нажмите для списка' : 'потерь связи нет'} onClick={() => store.nav('devices', 'down')} />
        <KpiTile label="Деградация связи" value={degraded} accent="#dfa65e" delay={120} icon={<Activity className="h-[18px] w-[18px]" />}
          sub={degraded ? 'пинг выше нормы в разы' : 'задержки в пределах нормы'} />
        <KpiTile label="Relay-агенты" value={`${agentsOnline}/${agents.length}`} accent="#7ba4e6" delay={180} icon={<Monitor className="h-[18px] w-[18px]" />}
          sub={agents.length ? `${agentsOnline} ПК на связи` : 'агенты не добавлены'} onClick={() => store.nav('agents')} />
      </div>
