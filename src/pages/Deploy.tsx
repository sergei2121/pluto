...
const AGENT_COMPONENTS = [
  ...
  { part: 'server/src/lib/relay.js', role: 'Ядро: координирует серии; RTT устройства = честный замер агента, путь ядро→хаб — отдельная метрика (hubRttMs)' },
...
#      latencyMs (честный локальный замер), min/max/jitter и потери серии.
#
# Диагностика:
        ...
        <div className="space-y-4">
          <Panel title="2 · Новый relay-агент на ПК" icon={<Monitor className="h-4 w-4" />}>
            <p className="mb-3 text-[12px] leading-relaxed text-dim">
              ...
              серии из 10 ICMP-пакетов (первый — warm-up), возвращая ядру медиану, min/max, джиттер и потери.
              Путь ядро→хаб измеряется отдельно (hubRttMs) и к RTT устройств не прибавляется — каждый IP показывает свой честный замер.
            </p>
