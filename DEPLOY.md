...
Установщик:
1. скачивает `PlutoAgent.cs` с ядра (`http://<IP>:8080/agent/PlutoAgent.cs`);
2. компилирует его `csc.exe` в `C:\ProgramData\pluto\pluto-agent.exe`;
3. создаёт службу Windows `pluto-agent` (автозапуск) и запускает её.
