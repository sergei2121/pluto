...
// (min/avg/max/медиана/jitter/потери). RTT устройства = честный локальный
// замер агента (pathMs == latency); путь ядро→хаб измеряется отдельно
// (hubRttMs) и к целям НЕ прибавляется — иначе все IP диапазона получали бы
// одинаковую надбавку и замеры выглядели бы эмулированными.
